...
class VerdictQueryScrapper:
    ...
    DOWNLOAD_TYPES = ['סוג מסמך: פסק-דין', 'סוג מסמך: החלטה', 'סוג מסמך: תקצירים', 'סוג מסמך: צו',
                      'סוג מסמך: צו על תנאי', 'סוג מסמך: צו ביניים', 'סוג מסמך: פסקי דין באנגלית', 'סוג מסמך: פד"י']
    ...
    async def scrap_for_file_links(self, search_input, file_type="pdf", delay=0) -> list[str]:
        ...
        judgement_elements = [sub_ele for sub_ele in sub_elements if
                              sub_ele.find_elements(By.TAG_NAME, 'span')[4].text == self.download_type]
        pdfs_links = []
