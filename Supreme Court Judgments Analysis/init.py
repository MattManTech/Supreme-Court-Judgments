from query_handler import VerdictQueryHandler
from judgment_scraper import VerdictQueryScrapper
from judgment_processor import VerdictProcessor
from query_processor import VerdictQueryBuilder
...
async def scrap_download(verdict_query_handler: VerdictQueryHandler, verdict_query_scrapper: VerdictQueryScrapper,
                         query_search_input: str, max_files: int):
...
async def queries_build(verdict_query_handler: VerdictQueryHandler, verdict_processor: VerdictProcessor):
    ...
    print_query = True
    processed_files = await verdict_builder.build(print_query=print_query, queries_indexes=queries_indexes,
                                                  query_files_indexes=query_files_indexes)
...
async def nlp(verdict_builder: VerdictQueryBuilder):
    ...
    taged_processed_queries_files = await compute_tf_idf_verdicts(vhnlp=vhnlp,
                                                                  processed_queries_files=verdict_builder.processed_queries_files)
...
async def run(config: Config):
    ...
    # init
    verdict_query_handler = VerdictQueryHandler(data_dir=data_dir, query_data_dir=query_data_dir,
                                                query_processed_data_dir=query_processed_data_dir,
                                                query_test_input=query_test_input)
    verdict_query_scrapper = VerdictQueryScrapper(chromedriver_path=chromedriver_path)
    verdict_processor = VerdictProcessor(data_dir=data_dir, query_data_dir=query_data_dir,
                                         query_processed_data_dir=query_processed_data_dir)
