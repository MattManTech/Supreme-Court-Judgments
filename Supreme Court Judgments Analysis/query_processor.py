...
class VerdictQueryBuilder():
    ...
    async def build_query(self, query, indexes=None, print_file=True):
        ...
        for i, query_file in enumerate(query_files):
            if indexes is not None:
                if i not in indexes:
                    continue
            file_text = await read_data_file_text(self.vedict_processor.data_dir, self.vedict_processor.query_data_dir,
                                                  query, query_file)
            if not file_text:
                return
    ...
    async def build(self, queries_indexes=None, query_files_indexes=None, print_query=True):
        processed_queries_files = await self.build_queries(indexes=queries_indexes, query_indexes=query_files_indexes,
